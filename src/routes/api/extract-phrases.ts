...
export const apiExtractPhrases = app
	.get(
		'/:id',
		zValidator(
			'query',
	...
	.post(
		...
		async (c) => {
			...
			const extractPhrases = createExtractPhrases(serviceBindingsMock(c.env).Calibrate, c.req.raw);
			// ルートの何処かですでにリクエストがクローンされている
			const phrases = await extractPhrases({ text, type: 'extract-phrases' }, true);
